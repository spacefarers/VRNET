    ...
    if stage == "source" or stage == "all":
        ...
        for source_iter in tqdm(range(source_iters),leave=False,desc="Source Training", position=0):
            ...
            for batch_idx, (low_res_source, high_res_source) in enumerate(tqdm(source_data, leave=False, desc="Source Iters", position=1)):
                ...
                M.module.restorer.requires_grad_(True)
                M.module.LR_domain_classifier.requires_grad_(False)
                M.module.feature_domain_classifier.requires_grad_(False)
                optimizer.zero_grad()
                features_S = M.module.encoder(low_res_source[:, 0:1], low_res_source[:, -1:])
                features_T = M.module.encoder(target_low[:, 0:1], target_low[:, -1:])
                # feature_source_label = M.module.feature_domain_classifier(features_S)
                # feature_target_label = M.module.feature_domain_classifier(features_T)
                ...
                source_restore = M.module.restorer(features_S)
                target_restore = M.module.restorer(features_T)
                # LR_target_label = M.module.LR_domain_classifier(target_low)
                # LR_source_label = M.module.LR_domain_classifier(source_restore)
                ...
                cycle_source_feature = M.module.encoder(source_restore[:, 0:1], source_restore[:, -1:])
                cycle_source_hi = M.module.upscaler(cycle_source_feature)
                source_hi = M.module.upscaler(features_S)
