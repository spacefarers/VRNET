...
def DomainAdaptation(run_id=200, source_iters=10, target_iters=10, tag="DA", disable_jump=True):
    ...
    if os.path.exists(f"{config.experiments_dir}/{config.run_id:03d}/source_trained.pth") and not disable_jump:
        ...
        M.load_state_dict(torch.load(f"{config.experiments_dir}/{config.run_id:03d}/source_trained.pth"))
    else:
    ...
    # Freeze upper layers
    for param in M.conv1.parameters():
    ...
    for target_iter in range(target_iters):
        ...
        vol_loss_total = 0
        for batch_idx, (low_res_source, high_res_source) in tqdm(target_data, leave=False, desc="Target Training"):
            optimizer_G.zero_grad()
            low_res_source = low_res_source.to(config.device)
            high_res_source = high_res_source.to(config.device)
    ...
    PSNR, PSNR_list = infer_and_evaluate(M, write_to_file=True)
    save_plot(PSNR, PSNR_list, config.experiments_dir + f"/{config.run_id:03d}", run_cycle=1)
    torch.save(M.state_dict(), f"{config.experiments_dir}/{config.run_id:03d}/target_trained.pth")
