...
class VAE(nn.Module):
    def __init__(self):
        ...
        self.enc_fc1 = nn.Linear(32 * np.prod(config.crop_size) * config.scale ** 3, 2048)
        self.enc_fc2_mean = nn.Linear(2048, 1024)
        self.enc_fc2_logvar = nn.Linear(2048, 1024)

        # Decoder
        self.dec_fc1 = nn.Linear(1024, 2048)
        self.dec_fc2 = nn.Linear(2048, 32 * np.prod(config.crop_size) * config.scale ** 3)
